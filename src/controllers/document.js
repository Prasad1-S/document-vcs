    ...
    if (data.rowCount == 0) {
      ...
      if (access == 'OWNER' || access == 'EDITOR' || access == 'VIEWER') {
        ...
        try {
          ...
          console.log(`Error Fetching the document and rendering the ejs: ${err}`);
        }
      }
    }
...
export async function ShowEditPage(req, res) {
  ...
  try {
    ...
    console.log(`Error fetching the document & rendering Edit Page: ${err}`);
  }
...
export async function DocumentVersionRollback(req, res) {
  ...
  try {
    ...
    /* Insert new version (rollback commit) */
  ...
  try {
    ...
    if (access.rowCount == 0 || access.rows[0].role == "VIEWER") {
      ...
      try {
        ...
        console.error(` Dataabase Error Creating a new version: ${err}`);
    ...
    console.log(`Error creating a new version of document: ${err}`);
